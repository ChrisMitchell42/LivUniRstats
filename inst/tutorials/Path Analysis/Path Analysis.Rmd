---
title: "More Regression & Path Analysis in R"
output: learnr::tutorial
runtime: shiny_prerendered
bibliography: CRG.bib
---
...
This tutorial follows on from the PGLS regression tutorial.

```{r, echo=TRUE, eval=FALSE}
learnr::run_tutorial("PGLS", package = "LivUniRstats")
```

In this tutorial, we will look at using multiple predictor variables in regression analyses. This can be a powerful approach but is not without caveats. We will look at one solution to the problem of variance inflation; path analysis.

## Topic 1: PGLS with Multiple Predictors

Let's have a closer look at using PGLS regression in statistical analyses. We'll be using the same primate data and tree from the PGLS tutorial.
...
If we have a closer look at the data, we notice a number of continuous variables describing primate life history. Let's say we're interested in what makes some primates live such long lives. The natural thing to do, is run a PGLS regression with longevity as the outcome.
```{r, echo=TRUE, eval=TRUE}
names(primate.data)
```

First, we need a comparative data object combining our data and tree. Revisit the PGLS tutorial for more information about these objects if needed and then create one using the function comparative.data().
```{r comp_obj, exercise=TRUE, exercise.eval = FALSE}
...
Each of those variables will need to be log transformed in our model. We can do this ahead of time to make the code easier. Note that in the code here, I'm working on the data stored in the comparative data object and not the imported tables.
```{r, echo=TRUE, eval=TRUE}
...
Now we can run the model just as we did in the PGLS tutorial.
```{r, echo=TRUE, eval=TRUE}
...
Our new output presents a similar picture. Body mass is significantly predicting longevity. Our new smaller model excludes the unimportant effect of gestation here. We're doing this manually step-by-step but you may want to investigate Lasso regression as a solution here. Least Absolute Shrinkage and Selection Operator (LASSO) regression is a good way to automate some of this process.

```{r, echo = FALSE, eval=TRUE, out.width="90%", fig.cap="Image credit: Chelsea Parlett-Pelleriti ($@$ChelseaParlett)"}
knitr::include_graphics('images/babyyoda.jpg')
```

Now our model looks like this:

$$ \text{Longevity = Body Mass + Home Range + Group Size}$$

Based on the output of m2, try and reduce the model further by dropping another term and see how it turns out! Remember to drop the least important predictor term in your model.

```{r model_reduction, exercise=TRUE, exercise.eval=FALSE}
m3 <- pgls()
```
```{r model_reduction-hint}
m3 <- pgls(log.lifespan ~ log.mass + log.range, data = primates, lambda = "ML")
```
...
```{r model_reduction2, exercise=TRUE, exercise.eval=FALSE}
m4 <- pgls()
```
```{r model_reduction2-hint}
m4 <- pgls(log.lifespan ~ log.mass, data = primates, lambda = "ML")
```
...
## Topic 2: Variance Inflation

In analyses with multiple predictor variables, it is not uncommon to run into problems. When constructing a regression model, it is important to make sure your predictor variables don't correlate.
```{r, echo=F, eval=TRUE, message=FALSE, results="hide", warning=FALSE}
ggplot(data = primate.data) +
  geom_point(aes(x = AdultBodyMass_g, y = GestationLen_d), cex = 0.75) +
...
The function **vif** in the package **car** calculates the variance inflation factor (VIF) of a model. Here we can see that every variable in our model has a VIF greater than 1, indicating we do have some multicollinearity. However, there is no critical value beyond which variance inflation becomes a problem. As a general rule of thumb, a value that exceeds 5 is considered enough of a problem that you shouldn't trust your results.
...
## Topic 3: Distance Separation

Confirmatory path analysis was developed to test causal hypotheses that can be represented by **Directed Acyclic Graphs** (DAGs) [@Shipley00b]. DAG is the term given to the graphical representation of the variables and the hypothesised relationships between them. We can test if a DAG is a reasonable hypothesis using a principle called **distance-separation** or d-sep for short [@Shipley00b].
...
```{r, echo = FALSE, eval=TRUE, out.width="30%", fig.cap="Example of a simple directed acyclic graph from Gonzalez-Voyer and Von Hardenberg 2014."}
...
The principle of distance separation allows us to test which variables are independent (ie. not related) and so we can use it to select an appropriate DAG model.

## Topic 4: Phylogenetic Path Analysis

So far, I've taken you through confirmatory path analysis but I haven't mentioned phylogentic control at all. Path analysis without phylogenetic control is performed using generalised linear models (glms). For phylogenetically controlled path analysis, we need PGLS!

Let's see how this works with an example.

```{r, echo=F, eval=TRUE}
rhino.dat <- read.csv("data/rhino.csv")
rownames(rhino.dat) <- rhino.dat$SP
rhino.tree <- read.tree("data/rhino.tree")
```
...
### Step 1: Define models to test

The first step is to define the models using the function **define_model_set**. Take your time over this! Typos will be very important. In this instance, let's reproduce the example DAG from the previous section. The formulae are passed in the same style as any formula in R.
```{r, echo = FALSE, eval=TRUE, out.width="30%", fig.cap="Example of a simple directed acyclic graph from Gonzalez-Voyer and Von Hardenberg 2014."}
knitr::include_graphics('Images/DAG1.png')
```
...
Each causal relationship from the diagram is represented here. Take a moment to check each formula matches the diagram.
...
The layout isn't exactly the same as the previous diagram but we can manipulate that with the argument **manual_layout** if we wish. We need to create a dataframe with x and y coordinates for the final plots. this is generally a good idea if you want compare multiple models. It will keep the orientations of the vertices constant for your plots. We can also set the arrows to be straight by setting **curvature** to be 0.
...
Because this is a complex step, __ALWAYS__ check your Directed Acyclic Graphs (DAGs) describing your models look as you expect them to.
...
### Step 2: Distance Separation

If we are doing this by hand, now we need to identify d-separation statements and test conditional independencies. **Phylopath** does it all for us. To interpret the final results, you really need to understand what happens in this step so take it slow and revisit the explanations in Topic 3. You can also refer to the methods papers which explain things very well [@Hardenberg13;@Gonzalez14].

The outcome of this process will be that some of our models will be rejected because the vertices in them are not truly independent.

### Step 3: Analysis

The phylopath package gives us the phylo_path function which will run the entire analysis in one go! We need to pass our data, tree and the model for regression.
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
...
knitr::kable(summary(results), digits = 3, row.names = FALSE) %>% 
